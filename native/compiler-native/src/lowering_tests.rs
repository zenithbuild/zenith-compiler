...
mod tests {
    ...
    use crate::validate::{TemplateNode, ExpressionIR, SourceLocation};
    use serde_json::json;
    ...
    fn test_conditional_lowering() {
        ...
        let result_json = lower_fragments_native(nodes_json, expressions_json, "test.zen".to_string()).unwrap();
    ...
    fn test_loop_lowering_with_context() {
        ...
        assert!(loop_ctx["variables"].as_array().unwrap().iter().any(|v| v == "item"));
    }
